...
if (! empty($conf->commande->enabled)) {
	...
	$var = ! $var;
	print '<tr ' . $bc[$var] . '>';
	print '<td>' . $langs->trans("set_GRAPEFRUIT_ORDER_CLASSIFY_SHIPPED_SHIPPING") . '</td>';
	print '<td align="center" width="20">&nbsp;</td>';
	print '<td align="right" width="300">';
	print '<form method="POST" action="' . $_SERVER['PHP_SELF'] . '">';
	print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
	print '<input type="hidden" name="action" value="set_GRAPEFRUIT_ORDER_CLASSIFY_SHIPPED_SHIPPING">';
	echo ajax_constantonoff('GRAPEFRUIT_ORDER_CLASSIFY_SHIPPED_SHIPPING');
	print '</form>';
	print '</td></tr>';
}
